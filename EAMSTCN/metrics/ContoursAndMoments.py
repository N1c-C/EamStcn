...
def get_contour_array(mask, bound_th=0.5):
    """ Returns one pixel wide contours of mask as a bit ndarray
    :param mask: A ground truth or predicted 2D (grayscale) image mask (H x W )
    :param bound_th: The pixel threshold value for a one or zero assignment
    """

    mask = torch.argmax(mask, dim=0).detach().numpy()[0].astype(np.uint8)

    plt.imshow(mask)
    plt.show()

    # Tests the mask is only 2 dimensional H X W. Raise AssertionError otherwise
    # assert len(torch.argmax(mask, dim=0)[0].shape) == 2, "Foreground mask should be 2D (HxW)"
...
def match_shape(mask, mask1):
    mask = torch.argmax(mask, dim=0).detach().cpu().numpy()[0].astype(np.uint8)
    mask1 = torch.argmax(mask1, dim=0).detach().cpu().numpy()[0].astype(np.uint8)

    # d1 = cv.matchShapes(mask, mask1, cv.CONTOURS_MATCH_I1, 0)
    d2 = cv.matchShapes(mask, mask1, cv.CONTOURS_MATCH_I2, 0)
    # d3 = cv.matchShapes(mask, mask1, cv.CONTOURS_MATCH_I3, 0)
    return d2

def get_moments(mask):
    """Returns the HU moments of a mask """
    print(cv.moments(get_contour_array(mask)))
    # return cv.moments(get_contour_array(mask))
