...
def save_checkpoint(state, dir, filename='unet_checkpoint.pth.tar'):
...
def get_loaders(train_dir, train_maskdir, val_dir, val_maskdir,
                batch_size, train_transform, val_transform, num_workers=2, pin_memory=True):
    """pin memory makes sure cpu to gpu copies are not on disk.
    Workers are used to  loader data ready in the cpu ram """
...
def save_predictions_as_images(loader, model, folder='saved_images/', device='cpu'):
    model.eval()
